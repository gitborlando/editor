...
export class Matrix {
  ...
  static applyXY = (xy: IXY, isInvert?: 'invert') => {
    const { x, y } = xy
    if (isInvert) {
      const [a, b, c, d, tx, ty] = this.matrix
      const invDet = 1 / (a * d - b * c)
  ...
  static applyAABB = (aabb: AABB, isInvert?: 'invert') => {
    const { minX, minY, maxX, maxY } = aabb
    const xy1 = Matrix.applyXY(XY._(minX, minY), isInvert)
    const xy2 = Matrix.applyXY(XY._(maxX, minY), isInvert)
    const xy3 = Matrix.applyXY(XY._(maxX, maxY), isInvert)
    const xy4 = Matrix.applyXY(XY._(minX, maxY), isInvert)
    return {
  ...
  static invertXY = (xy: IXY) => {
    return Matrix.applyXY(xy, 'invert')
  }

  static invertAABB = (aabb: AABB) => {
    return Matrix.applyAABB(aabb, 'invert')
  }

  static identity() {
    return Matrix.of(1, 0, 0, 1, 0, 0)
  }
